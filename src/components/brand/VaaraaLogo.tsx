  ...
  const content = (
    <div className={cn("inline-flex flex-col items-start leading-none", className)}>
      ...
      {showTagline && (
        <span
          className={cn(
            "font-sans uppercase font-medium text-muted-foreground/80 tracking-[0.22em] mt-0.5",
            size === "sm" && "text-[9px]",
            size === "md" && "text-[10px]",
            size === "lg" && "text-xs",
          )}
  ...
  if (href) {
    return (
      <Link
        ...
        aria-label="Vaaraa — See Your Wedding Look"
      >
        {content}
      </Link>
    );
...
export function VaaraaMonogram({ className }: { className?: string }) {
  return (
    <div
      className={cn(
